...
namespace LinePutScript
{
        ...
        where V : ISetObject, new()
    {
        ...
        /// 集合 全部文件的数据
        /// </summary>
        public List<TLine> Assemblage { get; set; } = new List<TLine>();
        ...
        {
            TLine oldline = FindLine(newLine.Name);
            if (oldline != null)
            {
                oldline.Set(newLine);
            }
        ...
        {
            foreach (TLine li in Assemblage)
                if (li.Contains(sub))
                    return true;
            return false;
        }
        ...
        public TLine this[int index] { get => Assemblage[index]; set => Assemblage[index] = value; }
        ...
        /// <returns>如果找到相同名称的Line的第一个元素,则为该元素的从零开始的索引; 否则为 -1</returns>
        public int IndexOf(TLine line) => Assemblage.FindIndex(x => x.Equals(line));
        /// <summary>
        /// 将指定的Line添加到指定索引处
        /// </summary>
        /// <param name="index">应插入 Line 的从零开始的索引</param>
        /// <param name="newLine">要添加的Line</param>
        public void Insert(int index, TLine newLine) => InsertLine(index, newLine);
        /// <summary>
        /// 将指定的Line添加到Assemblage列表的末尾
        /// </summary>
        /// <param name="newLine">要添加的Line</param>
        ...
        public void CopyTo(TLine[] array, int arrayIndex) => Assemblage.CopyTo(array, arrayIndex);
